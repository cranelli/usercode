...
if options.local :
    ...
    #Removing EOS storage

    command_base = 'python scripts/filter.py  --files root://eoscms/%(base)s/%(job)s.root --outputDir /afs/cern.ch/work/c/cranelli/public/WGamGam/%(output)s/%(job)s --outputFile tree.root --treeName %(treename)s --module scripts/%(module)s --enableKeepFilter --confFileName %(job)s.txt --nsplit %(nsplit)d --nproc %(nproc)d --exeName %(exename)s_%(job)s --moduleArgs "{ \'sampleFile\' : \'root://eoscms/%(base)s/%(job)s.root\'}"'
    
else :
    #--------------------
    # for batch submission
    #--------------------
    command_base = 'python scripts/filter.py  --files root://eoscms/%(base)s/%(job)s.root --outputDir /afs/cern.ch/work/c/cranelli/public/WGamGam/%(output)s/%(job)s --outputFile tree.root --treeName %(treename)s --module scripts/%(module)s --enableKeepFilter --batch --confFileName %(job)s.txt --nsplit %(nsplit)d --exeName %(exename)s_%(job)s --moduleArgs "{ \'sampleFile\' : \'root://eoscms/%(base)s/%(job)s.root\'}"'

if options.resubmit :
    command_base += ' --resubmit '

clean_command_base = ' python ../../Util/scripts/clean_conf_files.py --path /eos/cms/store/user/cranelli/Wgamgam/%(output)s/%(job)s '

check_commands_base = 'python ../../Util/scripts/check_dataset_completion.py --originalDS %(base)s --filteredDS /afs/cern.ch/work/c/cranelli/public/WGamGam/%(output)s/%(job)s --treeNameOrig %(treename)s --histNameFilt ggNtuplizer/filter --fileKeyOrig %(job)s.root --fileKeyFilt tree.root'

module = 'ConfWgamgamReco.py'
#module = 'ConfWgamgamRecoJetTrig.py'
#module = 'ConfWgamgamRecoWithoutTrig.py'
